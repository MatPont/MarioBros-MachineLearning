...
static const char * pythonLibName = "libpython3.6m.so";
...
const char* defaultAgentName = "AmiCo Python Agent";

void printPythonError2(){
	...
	if(pvalue != NULL){
		...
		std::cout << "pvalue is null" << std::endl;
        if(ptraceback != NULL){
                char *p2StrErrorMessage = PyBytes_AsString(ptraceback);
                std::cout << p2StrErrorMessage << std::endl;
        }else
             	std::cout << "ptraceback is null" << std::endl;
...
Java_ch_idsia_tools_amico_AmiCoJavaPy_initModule(JNIEnv* env,
                                            ...
                                            jstring classNameJ)
{
	dlopen(pythonLibName, RTLD_LAZY | RTLD_GLOBAL);
	...
	//PySys_SetPath(L"/users/smac/mpont/mario-DQN/src/main/bin/AmiCoBuild/JavaPy");
	...
	memset(str, 0, 256);
	wchar_t* array = new wchar_t[256];
	array = Py_GetPath();
	wcstombs(str, array, 256);
	...
	std::cout << moduleName << " _ " << mainModule << std::endl;

    if (moduleName != NULL && mainModule != NULL)
...
Java_ch_idsia_tools_amico_AmiCoJavaPy_getName(JNIEnv* env, jobject obj)
{
    ...
    if (res == NULL)
    {
        std::cerr << AMICO_ERROR << "Method getName has returd nothing. Using default agent name: " << defaultAgentName << std::endl;
        return env->NewStringUTF(defaultAgentName);
    }
    if (!PyBytes_Check(res))
    {
        std::cerr << AMICO_ERROR << "Object return by getName method is not a string. Using default agent name: " << defaultAgentName << std::endl;
        return env->NewStringUTF(defaultAgentName);
    }
...
Java_ch_idsia_tools_amico_AmiCoJavaPy_getAction(JNIEnv* env, jobject obj)
{
    ...
    for (int i = 0; i < size; i++)
        ar[i] = PyLong_AsLong(PyTuple_GetItem(res, i));
...
Java_ch_idsia_tools_amico_AmiCoJavaPy_setObservationDetails(JNIEnv* env,
                                                            ...
                                                            jint egoCol)
{
    PyObject* width = PyLong_FromLong(rfWidth);
    PyObject* height = PyLong_FromLong(rfHeight);
    PyObject* row = PyLong_FromLong(egoRow);
    PyObject* col = PyLong_FromLong(egoCol);
