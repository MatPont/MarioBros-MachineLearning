...
maskHalfLoss = False # Mask the first half loss of every trace of the batch
reset_rnn_state = True # LSTM stateless (True) or stateful (False) (stateless: reset state after every batch, stateful: keep state)
useLSTMTanH = False # Allows to use TanH for LSTM or the activation define in build_network() function 
...
class MarioDQNAgent():
	...
	def __init__(self):
		...
		"""for param in tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='prediction_network'):
			print(param)		
		for param in tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='target_network'):
			print(param)
		input()"""
	...
	def initAgent(self,
		...
		# Frequencies
		self.compute_action_frequency = 1 # Step before computing another action (4 is often used)
		self.target_network_update_frequency = 2000 / self.compute_action_frequency # how many minibatch steps before we set the target network weights to the prediction network weights
	...
	def build_before_last_fc_layer(self, inputs, num_units, scope_name, activation):
		...
		if not useLSTM:
			...
			with tf.variable_scope('lstm') as scope:
				...
				self.rnn_trainLength[scope_name] = tf.placeholder(dtype=tf.int32)
				# We take the output from the final convolutional layer and send it to a recurrent layer.
				# The input must be reshaped into [batch x trace x units] for rnn processing, 
				# and then returned to [batch x units] when sent through the upper levels.
				self.rnn_batch_size[scope_name] = tf.placeholder(dtype=tf.int32, shape=[])
				rnn_convFlat = tf.reshape(inputs,[self.rnn_batch_size[scope_name], self.rnn_trainLength[scope_name], inputs.get_shape()[1]])
				self.rnn_state_in[scope_name] = rnn_cell.zero_state(self.rnn_batch_size[scope_name], tf.float32)
				rnn, self.rnn_state[scope_name] = tf.nn.dynamic_rnn(\
					inputs=rnn_convFlat, cell=rnn_cell, dtype=tf.float32, initial_state=self.rnn_state_in[scope_name])
	...
	def build_last_fc_layer(self, inputs, num_units, scope_name, prediction_network):
		if not useDuelingNetwork:					
			...
			with tf.variable_scope('fc2') as scope:
				...
				salience = tf.gradients(Advantage, self.network_inputs[scope_name])
